...
- `hermes.service` — the **only** systemd unit in this repository.
  ...
  activation is an explicit operator step after Milestone 2 connection
  setup succeeds; it is exercised only during the approved Fresh EC2 plan.
...
(`hermes gateway run`), not a separate service — see `ARCHITECTURE.md` §6/§9.
